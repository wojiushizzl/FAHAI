...
class FAHAI():
    ...
    def entry_project(self, e):
        ...
        self.images_count = function.count_image_files(
            os.path.join(os.getcwd(), 'projects', str(self.selected_project), 'datasets', 'images'))
        self.labels_count = function.count_txt_files(
            os.path.join(os.getcwd(), 'projects', str(self.selected_project), 'datasets', 'labels'))
        self.classfile_exist = function.find_file(
            os.path.join(os.getcwd(), 'projects', str(self.selected_project), 'datasets',
    ...
    def make_projects_gridview(self):
        ...
        for i, p in enumerate(projects_list):
            ...
            self.images.controls.append(
                self.project_card(icon=ft.icons.AUTO_AWESOME, text=p, description=description, action1='Entry',
                                  action2='Delete')
            )
        return self.images
